...
int hitingpoint (Mat &houghImage, vector<Vec4f> &circles, int &whiteBall, int &dstBall, Point dstAimingPoint);
...
int hitingpoint (Mat &houghImage, vector<Vec4f> &circles, int &whiteBall, int &dstBall, Point &dstAimingPoint)
{
    ...
    for (int i = 0; i < 6; i++){
        //利用相似原理，求出目标瞄准点的位置
        aimingPoint[i].x = circles[dstBall][0] + (distance / sqrtf(powf(circles[dstBall][0] - aimingPoint[i].x, 2) 
                + powf(circles[dstBall][0] - aimingPoint[i].x, 2))) * (circles[dstBall][0] - aimingPoint[i].x);
        aimingPoint[i].y = circles[dstBall][1] + (distance / sqrtf(powf(circles[dstBall][0] - aimingPoint[i].x, 2) 
                + powf(circles[dstBall][0] - aimingPoint[i].x, 2))) * (circles[dstBall][1] - aimingPoint[i].y);
